...
async function buildPayLine(order, customer, { amount, amountLabel }) {
  ...
  if (paymentConfig?.provider === 'monnify') {
    try {
      // Chidera: "the monnify account that was sent is unavailable and
      // invalid and cant it be a link like paystack? so the auto confirm
      // can be obvious." A real hosted checkout link now

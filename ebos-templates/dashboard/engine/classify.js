...
// it less and still be natural" -- all three functions below only ever
// return a fixed-label routing decision (which code path runs next), never
// text the customer sees, so Haiku classifies them just as reliably as
// Sonnet for a fraction of the cost. See claude.js's own top comment for
// the full reasoning and what's deliberately NOT changed.
export async function classifyIntent(message) {
  const result = await askJson(SYSTEM, message, { useHaiku: true });
...
export async function detectWantsHuman(message) {
  const result = await askJson(WANTS_HUMAN_SYSTEM, message, { useHaiku: true });
...
export async function detectDelayComplaint(message) {
  const result = await askJson(DELAY_COMPLAINT_SYSTEM, message, { useHaiku: true });
